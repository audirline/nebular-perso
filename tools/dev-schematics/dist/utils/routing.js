...
exports.isComponentRoute = exports.isLazyRoute = exports.rootRoutePredicate = exports.baseComponentPredicate = exports.lazyModulePredicate = exports.componentRoutePredicate = exports.pathRoutePredicate = exports.routePredicatesFromPath = exports.dirsToRoutePaths = exports.getRoutePath = exports.getRouteComponent = exports.getRouteLazyModule = exports.findRouteProp = exports.getRouteProps = exports.getRouteChildren = exports.getRoutesFromArray = exports.findRouteWithPath = exports.findRoute = exports.addRoute = exports.addBaseRoute = exports.addRootRoute = exports.addMissingChildRoutes = exports.generateLazyModuleImport = exports.generateRoute = exports.generatePathRoute = exports.generateComponentRoute = exports.findRoutesArray = void 0;
...
function getRouterModuleCall(importsNode) {
    const routerModuleCall = importsNode.initializer.elements
...
function getRoutesVariableDeclaration(source, identifier) {
    const declaration = (0, utils_1.findDeclarationByIdentifier)(source, identifier.getText());
    if (declaration == null) {
...
function generateLazyModuleImport(from, to, moduleClassName) {
    const path = (0, core_1.normalize)((0, utils_1.importPath)(from, to));
    return `() => import('./${(0, core_1.dirname)(path)}/${(0, core_1.basename)(path)}').then(m => m.${moduleClassName})`;
}
exports.generateLazyModuleImport = generateLazyModuleImport;
/**
...
function addMissingChildRoutes(tree, routingModulePath, targetFile) {
    const routingModuleDir = (0, core_1.dirname)(routingModulePath);
    if ((0, utils_1.isRootPlaygroundModule)(routingModuleDir)) {
        addRootRoute(tree, targetFile);
        return;
    }
    if ((0, utils_1.isBasePlaygroundModule)(routingModuleDir)) {
...
function addMissingPaths(tree, routingModulePath, targetFile) {
    const targetDir = (0, core_1.dirname)(targetFile);
    const relativePath = targetDir.replace((0, core_1.dirname)(routingModulePath), '');
    const existingPathEnd = targetDir.indexOf(relativePath);
    let existingPath = (0, core_1.normalize)(targetDir.slice(0, existingPathEnd));
    const pathsToCheck = dirsToRoutePaths(relativePath);
    for (let i = 0; i < pathsToCheck.length; i++) {
        const routePathToCheck = pathsToCheck[i];
        const fullPathToCheck = (0, core_1.join)(existingPath, routePathToCheck);
        ...
        if (!getRouteChildren(route)) {
            (0, utils_1.addObjectProperty)(tree, (0, schematics_2.parseSourceFile)(tree, routingModulePath), route, 'children: []');
        }
    }
}
function getParentRouteChildren(routesArray, routingModulePath, targetDir) {
    const predicates = routePredicatesFromPath(routingModulePath, (0, core_1.dirname)(targetDir));
...
function addRootRoute(tree, targetFile) {
    ...
    const isLayout = (0, utils_1.isLayoutPath)(targetFile);
    ...
    const lazyModuleImport = generateLazyModuleImport(utils_1.PLAYGROUND_ROUTING_MODULE_PATH, baseModulePath, baseModuleClass);
...
function addBaseRoute(tree, targetFile) {
    ...
    if (baseRoute) {
        if (!getRouteChildren(baseRoute)) {
            (0, utils_1.addObjectProperty)(tree, (0, schematics_2.parseSourceFile)(tree, baseModulePath), baseRoute, 'children: []');
        }
        return;
    }
...
function addRoute(tree, routingModulePath, routes, route, componentClass, fileImportPath) {
    const source = (0, schematics_2.parseSourceFile)(tree, routingModulePath);
    const alreadyInRoutes = routes.getFullText().includes(route);
    if (alreadyInRoutes) {
        return;
    }
    (0, utils_1.addArrayElement)(tree, source, routes, route);
    if (componentClass && fileImportPath) {
        const importChange = (0, schematics_2.insertImport)(source, source.fileName, componentClass, fileImportPath);
        (0, utils_1.applyInsertChange)(tree, (0, core_1.normalize)(source.fileName), importChange);
    }
}
...
function getRoutesFromArray(routesArray) {
    return routesArray.elements.filter((node) => node.kind === ts.SyntaxKind.ObjectLiteralExpression);
}
...
function getRouteProps(route) {
    return route.properties.filter((prop) => prop.kind === ts.SyntaxKind.PropertyAssignment);
}
...
function routePredicatesFromPath(routingModulePath, targetDirPath) {
    ...
    const relativeToRoutingModule = targetDirPath.replace((0, core_1.dirname)(routingModulePath), '');
...
function lazyModulePredicate(lazyModuleImport, route) {
...
function baseComponentPredicate(modulePath) {
    const rootComponentClass = (0, utils_1.isLayoutPath)(modulePath) ? utils_1.LAYOUT_COMPONENT_CLASS : utils_1.NO_LAYOUT_COMPONENT_CLASS;
...
function rootRoutePredicate(modulePath) {
    const isLayout = (0, utils_1.isLayoutPath)(modulePath);
    const baseModulePath = isLayout ? utils_1.LAYOUT_MODULE_PATH : utils_1.NO_LAYOUT_MODULE_PATH;
    const baseModuleClass = isLayout ? utils_1.LAYOUT_MODULE_CLASS : utils_1.NO_LAYOUT_MODULE_CLASS;
    const lazyModuleImport = generateLazyModuleImport(utils_1.PLAYGROUND_ROUTING_MODULE_PATH, baseModulePath, baseModuleClass);
    return (route) => lazyModulePredicate(lazyModuleImport, route);
}
